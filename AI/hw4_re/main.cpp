...
queen* evolution_GA(queen* origin,int nqueen,int population,float mutationRate,int generation)
{
    ...
    while(generation--)
    {
        ...
        for(int i=0;i<population;i++)
        {
            if(origin[i].attack()>worstAttack)
            {
                worstAttack=origin[i].attack();
                worstPos=i;
            }
        }
        origin[worstPos]=temp;
        //mutation
        origin=mutation(origin,nqueen,population,mutationRate);
        //fitness
        for(int i=0;i<population;i++)
        {
